...
function permissive (tuple) {
  if (tuple.name === ROOT_NAME) return false;
  if (tuple.private) return false;
  let license = tuple.license;
...
Walker.prototype.addDictionary = function (name) {
  if (this.configs[name]) return;
...
Walker.prototype.activateConfig = function (tuple) {
  ...
  const base = tuple.base;
  if (!base) assert(false);
  const config = that.configs[name];
  if (!config) return;
...
Walker.prototype.hasDictionary = function (tuple) {
  if (!tuple) assert(false);
  const name = tuple.name;
  ...
  try {
    detector.detect(
      ...
      function (node, trying) {
        ...
        if (p) {
          if (p.dontEnclose) return false;
          if (that.hasDictionary(record.tuple)) return false;
...
Walker.prototype.stepDerivatives_ALIAS_AS_RESOLVABLE = function (record, derivative, cb) { // eslint-disable-line camelcase
  ...
  catcher.packageFilter = function (tuple, base) {
    if (tuple.name) {
      newTuple = tuple;
      newTuple.base = base;
    }
...
Walker.prototype.step_STORE_ANY = function (record, cb) { // eslint-disable-line camelcase
  ...
  if (record.store === STORE_CODE) {
    ...
    if (permissive(record.tuple) ||
        that.hasDictionary(record.tuple)) { // ejs 0.8.8 has no license field
      that.stack({
        file: record.file,
        tuple: record.tuple,
        store: STORE_CONTENT,
