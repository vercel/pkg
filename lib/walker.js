...
class Walker {
  ...
  async stepRead (record) {
    ...
    try {
      ...
      config = this.config.pkg;

      if (config && config.preprocessors) {
        config.preprocessors.forEach((preprocessor) => {
          const regex = new RegExp(preprocessor.test);
          if (regex.test(record.file)) {
            const transform = this.getTransformer(preprocessor.transform);
            body = transform(record.file, body);
          }
        });
      }
    } catch (error) {
      log.error('Cannot read file, ' + error.code, record.file, error);
  ...
  async stepDerivatives_ALIAS_AS_RESOLVABLE (record, marker, derivative) { // eslint-disable-line camelcase
    ...
    try {
      newFile = await follow(derivative.alias, {
        ...
        extensions: (this.config && this.config.pkg && this.config.pkg.extensions) ||
          [ '.js', '.json', '.node' ],
