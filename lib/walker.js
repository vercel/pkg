      ...
      if (typeof dependencies === 'object') {
        for (const dependency in dependencies) {
          derivatives.push({
            ...
            // once 'canIgnore' becomes 'warn' instead of 'error'
            // btw this is caused by `babel-runtime` `main` entry absence
            canIgnore: true
          });
        }
      }
...
Walker.prototype.stepDerivatives_ALIAS_AS_RESOLVABLE = async function (record, derivative) { // eslint-disable-line camelcase
  ...
  if (newTuple) {
    ...
    const { pkgConfig } = config; // TODO move this somewhere
    if (pkgConfig) {
      const { patches } = pkgConfig;
      if (patches) {
        for (const key in patches) {
          const p = path.join(newTuple.base, key);
          this.patches[p] = patches[key];
        }
      }
    }
    try {
      await this.appendFilesFromConfig(newTuple);
    } catch (error) {
      const newFile2 = path.join(newTuple.base, 'package.json');
      reporter.report(newFile2, 'error', error.message, error);
      throw error;
    }
  }
...
Walker.prototype.start = async function (opts) {
  ...
  const { tuple, primary, secondary } = opts;
  ...
  try {
    // TODO move appendFilesFromConfig to step_ANY
  ...
  for (let i = 0; i < records.length; i += 1) {
    ...
    // HANDLING ORDER. one-by-one fifo is the only way to iterate records
