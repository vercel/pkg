...
const solver = require('resolve').sync;
...
Walker.prototype.addConfigFromDictionary = function (name) {
  ...
  const dictionary = path.join(home, '..', 'dictionary', name + '.js');
  this.addConfigFromFile(name, dictionary, true);
};

const relativize = function (p, base) {
  if (typeof p !== 'string') {
    throw new Error(
...
Walker.prototype.stepDerivatives_ALIAS_AS_RESOLVABLE = function (record, derivative, cb) { // eslint-disable-line camelcase
  ...
  if (newPack) {
    ...
    try {
      ...
      const file3 = path.join(newPack.base, 'package.json');
...
Walker.prototype.start = function (opts, cb) {
  ...
  const input = opts.cli.input;
  const boot = opts.cli.config || opts.cli.input;
