...
function isPermissive (config) {
  ...
  if (Array.isArray(licenses)) {
    license = licenses.map((c) => String(c.type || c)).join(',');
  }
...
class Walker {
  ...
  stepDetect (record, derivatives) {
    ...
    try {
      detector.detect(body, (node, trying) => {
        const { fromDictionary } = record.marker;
        ...
        if (d) {
          if (d.mustExclude) return false;
          const debug = fromDictionary || d.mayExclude || trying;
        ...
        if (d) {
          ...
          const debug = fromDictionary || trying;
  ...
  async stepDerivatives_ALIAS_AS_RESOLVABLE (record, derivative) { // eslint-disable-line camelcase
    ...
    if (failure) {
      ...
      const { fromDictionary } = record.marker;
      const debug = fromDictionary || derivative.mayExclude || mainNotFound;
      const level = debug ? 'debug' : 'warn';
  ...
  async step_STORE_ANY (record, store) { // eslint-disable-line camelcase
    ...
    if (store === STORE_BLOB) {
      ...
      if (record.marker.permissive ||
          record.marker.fromDictionary) { // ejs 0.8.8 has no license field
        this.append({
          file: record.file,
          marker: record.marker,
          store: STORE_CONTENT
