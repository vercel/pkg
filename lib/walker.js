...
class Walker {
  ...
  append (task) {
    ...
    if (task.reason) {
      log.debug(what + ' %1 is added to queue. It was required from %2', [ task.file, task.reason ]);
    } else {
      log.debug(what + ' %1 is added to queue', [ task.file ]);
    }
  }
  ...
  async stepDerivatives (record, marker, derivatives) {
    for (const derivative of derivatives) {
      ...
      if (derivative.aliasType === ALIAS_AS_RELATIVE) {
        await this.stepDerivatives_ALIAS_AS_RELATIVE(record, record.marker, derivative);
      } else
      if (derivative.aliasType === ALIAS_AS_RESOLVABLE) {
        await this.stepDerivatives_ALIAS_AS_RESOLVABLE(record, record.marker, derivative);
      } else {
  ...
  async step_STORE_ANY (record, store) { // eslint-disable-line camelcase
    ...
    const derivatives1 = [];
    await this.stepActivate(record.marker, derivatives1);
    await this.stepDerivatives(record, record.marker, derivatives1);
    ...
    if (store === STORE_BLOB ||
      ...
      if (store === STORE_BLOB) {
        const derivatives2 = [];
        this.stepDetect(record, record.marker, derivatives2);
        await this.stepDerivatives(record, record.marker, derivatives2);
      }
    }
