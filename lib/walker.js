...
Walker.prototype.followAlias = function (file, tuple, alias, canIgnore) {

  const catcher = {};
  let newTuple = null;

  catcher.readFileSync = function (newFile1) {
  ...
  if (newTuple) {
    ...
    if (name) {
      try {
        const d = this.hasDictionary(name);
        if (d) {
  ...
  if (failure) {
    ...
    if (canIgnore) {
      reporter.report(file, 'info', error2.message);
      return;
    } else {
      reporter.report(file, 'error', error2.message, error2);
      throw error2;
    }
  }
...
Walker.prototype.stepRead = function (record) {
...
Walker.prototype.stepPatch = function (record) {
  ...
  if (isPackageJson(record.file)) return; // package.json is package-neutral
...
Walker.prototype.stepDerivatives_ALIAS_AS_RELATIVE = function (record, derivative) { // eslint-disable-line camelcase
...
Walker.prototype.stepDerivatives_ALIAS_AS_RESOLVABLE = function (record, derivative) { // eslint-disable-line camelcase

  await this.followAlias(
    record.file, record.tuple,
    derivative.alias, derivative.canIgnore
  );

};

Walker.prototype.stepDerivatives = function (record, derivatives) {
  ...
  async.map(derivatives, function (derivative, next) {
    ...
    if (derivative.aliasType === ALIAS_AS_RELATIVE) {
      this.stepDerivatives_ALIAS_AS_RELATIVE(record, derivative, next);
    } else
    if (derivative.aliasType === ALIAS_AS_RESOLVABLE) {
      this.stepDerivatives_ALIAS_AS_RESOLVABLE(record, derivative, next);
    } else {
...
Walker.prototype.step_STORE_ANY = function (record) { // eslint-disable-line camelcase
  ...
  async.waterfall([
    ...
    function (next) {
      this.stepRead(record, next);
    },
    function (next) {
      this.stepPatch(record, next);
    },
    function (next) {
    ...
    function (next) {
      this.stepStrip(record, next);
    },
    function (next) {
      this.stepDetect(record, next);
    },
    function (derivatives, next) {
      this.stepDerivatives(record, derivatives, next);
    }
...
Walker.prototype.step_STORE_STAT = function (record) { // eslint-disable-line camelcase
...
Walker.prototype.step = function (record) {
