...
Walker.prototype.appendFilesFromConfig = async function (tuple) {
  ...
  const pkgConfig = config.pkg || config.pkgConfig; // TODO remove pkgConfig soon
...
Walker.prototype.stepDetect = function (record, derivatives) {
  ...
  try {
    detector.detect(body, (node, trying) => {
      ...
      if (d) {
        if (d.mustExclude) return false;
        const level = ((rtd || d.mayExclude || trying) ? 'debug' : 'warn');
      ...
      if (d) {
        ...
        const level = ((rtd || trying) ? 'debug' : 'warn'); // there is no 'mayExclude'
...
Walker.prototype.stepDerivatives_ALIAS_AS_RELATIVE = async function (record, derivative) {
...
Walker.prototype.stepDerivatives_ALIAS_AS_RESOLVABLE = async function (record, derivative) {
  ...
  if (failure) {
    const rtd = record.tuple.dictionary;
    const level = ((rtd || derivative.mayExclude || mainNotFound) ? 'debug' : 'warn');
...
Walker.prototype.step_STORE_ANY = async function (record) {
...
Walker.prototype.step_STORE_LINKS = function (record) {
...
Walker.prototype.step_STORE_STAT = async function (record) {
