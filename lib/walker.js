import { ALIAS_AS_RELATIVE, ALIAS_AS_RESOLVABLE,
  ...
  isDotJSON, isDotNODE, isPackageJson, normalizePath
      ...
      for (const file of files) {
        ...
        if (stat.isFile()) {
          this.append({
            ...
            tuple: tuple,
            store: STORE_CONTENT,
            treatAsCODE: false // see test-50-config-json-7
          });
        }
      }
    }
...
Walker.prototype.stepDetect = function (record, derivatives) {
  ...
  try {
    detector.detect(body, (node, trying) => {
      ...
      if (p) {
        if (p.mustExclude) return false;
        p.mayExclude = p.mayExclude || trying;
        derivatives.push(p);
        return false;
      }
      ...
      if (p) {
        ...
        level = ((p.mayExclude || trying) ? 'info' : 'warning');
      ...
      if (p) {
        ...
        if (record.tuple.dictionary) return false;
        level = (trying ? 'info' : 'warning'); // there is no "mayExclude"
        reporter.report(record.file, level, [
      ...
      if (p) {
        // there is no "mustExclude"
        if (record.tuple.dictionary) return false;
        level = 'info'; // there is no "mayExclude"
        reporter.report(record.file, level, [
