  ...
  try {
    detector.detect(
      ...
      function(name, value, trying) {
        var p, level;
        p = detector.visitor_SUCCESSFUL(name, value);
        if (p) {
        ...
        p = detector.visitor_NONLITERAL(name, value);
        if (p) {
          if (p.dontEnclose) return false;
        ...
        p = detector.visitor_MALFORMED(name, value);
        if (p) {
        ...
        p = detector.visitor_USESCWD(name, value);
        if (p) {
          // there is no "dontEnclose"
          if (that.hasNonRootConfig(record.pack)) return false;
