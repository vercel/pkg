...
class Walker {
  ...
  async appendFilesFromConfig (tuple) {
  ...
  async stepDerivatives_ALIAS_AS_RESOLVABLE (record, derivative) { // eslint-disable-line camelcase
    ...
    assert((newPackage && newTuple) ||
           (!newPackage && !newTuple), 'Probably, package.json is malformed');
    ...
    if (failure) {
      let { message } = failure;
      if (mainNotFound) message = `Package.json.main entry not found in ${newPackage.name}`;
      const rtd = record.tuple.dictionary;
