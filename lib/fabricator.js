...
const script = `
  ...
  var stdin = SafeBuffer.alloc(0);
  process.stdin.on('data', function (data) {
    stdin = SafeBuffer.concat([ stdin, data ]);
    if (stdin.length >= 4) {
