...
const wasReported = require('./log.js').wasReported;
...
function producer (opts, cb) {
  ...
  try {
    ...
    log.error('Failed to spawn fabricator ' + fabricatorName, [
      fabricatorName,
      error.toString() ]);
    return cb(wasReported(error));
  }
  ...
  fabricator.on('exit', function (code, signal) {
    if (code !== 0) {
      const error = new Error('Fabricator exited with code/signal ' + (code || signal));
