...
function injectBakeryToBinary (bakes, binaryPath) {
  const parts = [];
  if (bakes.length) {
  ...
  const placeholder = Buffer.from('\0' + '// BAKERY '.repeat(20));
  const padding = Buffer.from('\0'.repeat(placeholder.length - buffer.length));
  ...
  assert(position !== -1, 'Bakery placeholder not found');
  buffer.copy(binaryBuffer, position);
  return binaryBuffer;
}
    ...
    multistream((cb) => {
      if (count === 0) {
        return cb(undefined, next(
          bufferStream(injectBakeryToBinary(bakes, target.binaryPath))
        ));
      } else
      if (count === 1) {
      ...
      fs.open(target.output, 'r+', (error, fd) => {
        ...
        fs.write(fd, buffer, 0, buffer.length, payloadPosition, (error2) => {
