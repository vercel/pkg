...
export default function ({ backpack, options, target, slash }) {
  return new Promise((resolve, reject) => {
    ...
    const snaps = stripes.map((stripe) => stripe.snap);
    const denominator = retrieveDenominator(snaps);
    ...
    for (const stripe of stripes) {
      let { snap } = stripe;
      snap = snapshotify(snap, denominator, slash);
        ...
        if (stripes.length) {
          ...
          if (stripe.buffer) {
            if (stripe.store === STORE_BLOB) {
              const snap = snapshotify(stripe.snap, denominator, slash);
