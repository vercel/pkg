...
export default function ({ backpack, options, target, slash }) {
  ...
  return new Promise((resolve, reject) => {
    let { prelude, entrypoint, stripe } = backpack;
    entrypoint = snapshotify(entrypoint, slash);
    stripe = stripe.slice();

    const vfs = {};
    for (const order of stripe) {
      const { file } = order;
      const snap = snapshotify(file, slash);
    ...
    multistream((cb) => {
      ...
      if (count === 3) {
        ...
        if (stripe.length) {
          const order = stripe.shift();
          recentOrder = order;

          if (order.buffer) {
            if (order.store === STORE_BLOB) {
              const snap = snapshotify(order.file, slash);
              return fabricate(options, target.fabricator, snap, order.buffer, (error, buffer) => {
                if (error) return cb(error);
                cb(undefined, pipeToNewMeter(bufferStream(buffer)));
              });
            } else {
              return cb(undefined, pipeToNewMeter(bufferStream(order.buffer)));
            }
          } else
          if (order.file) {
            assert.equal(order.store, STORE_CONTENT); // others must be buffers from walker
            return cb(undefined, pipeToNewMeter(fs.createReadStream(order.file)));
          } else {
            assert(false, 'producer: bad stripe order');
          }
        } else {
          return cb(undefined, next(
