import { STORE_BLOB, STORE_CONTENT, retrieveDenominator,
  snapshotify } from '../prelude/common.js';
...
export default function ({ backpack, options, target, slash }) {
  return new Promise((resolve, reject) => {
    ...
    const files = stripes.map((stripe) => stripe.file);
    const denominator = retrieveDenominator(files);
    entrypoint = snapshotify(entrypoint, denominator, slash);
    stripes = stripes.slice();

    const vfs = {};
    for (const stripe of stripes) {
      const { file } = stripe;
      const snap = snapshotify(file, denominator, slash);
        ...
        if (stripes.length) {
          ...
          if (stripe.buffer) {
            if (stripe.store === STORE_BLOB) {
              const snap = snapshotify(stripe.file, denominator, slash);
