...
function parseTargets (items) {
  // v6-macos-x64,v6-linux-x64
  if (!items) return [];
  if (Array.isArray(items)) {
    throw wasReported(`Something is wrong near ${items[0]}`);
  }
  const targets = [];
...
export async function exec (argv2) { // eslint-disable-line complexity
  ...
  let targets = parseTargets(argv.t || argv.target || argv.targets);
  ...
  for (const target of targets) {
    if (target.arch !== hostArch) {
      throw wasReported(`Not able to compile for '${target.arch}' here, only for '${hostArch}'`);
    }
  }
