...
export async function exec (argv2) { // eslint-disable-line complexity
  ...
  const records = refine(await walker({
    marker, entrypoint: inputFin
  }));

  const backpack = await packer({
    records, entrypoint: inputFin
  });
  ...
  for (const target of targets) {
    ...
    await producer({ backpack, options, target, slash });
