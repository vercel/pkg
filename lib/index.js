import { exists, mkdirp, readFile, stat } from 'fs-promise';
...
export async function exec (argv2) { // eslint-disable-line complexity
  ...
  if (!targets.length) {
    if (!autoOutput) {
      ...
      targets = parseTarget('linux,macos,win');
    }
  ...
  for (const target of targets) {
    ...
    await producer({ stripe: stripes[target.slash], target });
