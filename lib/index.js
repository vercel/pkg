...
export async function exec (argv2) {
  ...
  if (configPath) configPath = path.resolve(configPath);
  const boot = configPath ? configPath : path.dirname(input);
  let config = configPath ? require(configPath) : null;
  if (config) config = { pkgConfig: config };
  const records = await walker({ boot, config, input });
