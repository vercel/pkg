import { exists, readFile, writeFile } from 'fs-promise';
...
export async function exec (argv2) {
  ...
  if (!output) {
    output = 'project'; // TODO from package.json.name?
    autoOutput = true;
  }
  ...
  if (config) {
    ...
    const json = require(configPath); // may be either json or js
    tuple.config = { pkgConfig: json };
    tuple.base = path.dirname(configPath);
  }

  if (!tuple.config) {
    tuple.config = {};
    tuple.base = path.dirname(input);
  }

  const records = await walker({ tuple, input });
