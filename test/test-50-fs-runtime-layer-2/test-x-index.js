...
function test02() {
  ...
  var stats = fs.statSync(the_file);
  console.log("stats.size", stats.size);
  var fd = fs.openSync(the_file, "r");
  var fstats = fs.fstatSync(fd);
  console.log("fstats.size", fstats.size);
  var buffer = new Buffer(stats.size);
  var bytes_read = fs.readSync(fd, buffer, 0, buffer.length);
...
function test03() {
  ...
  var bytes_read = fs.readSync(fd, buffer, 0, 6);
...
function test04() {
  ...
  var stats = fs.statSync(the_directory);
  console.log(stats.size);
  var fd = fs.openSync(the_directory, "r");
  var fstats = fs.fstatSync(fd);
  console.log(fstats.size);
  var buffer = new Buffer([ 12, 34, 56, 78 ]);
  var bytes_read;
...
function test07() {
  ...
  var rs = fs.createReadStream(the_file);
